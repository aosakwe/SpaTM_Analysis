...
# Load Data
spe <- readRDS("./data/spe.rds")#readRDS("../data/spe.rds")
...
models <- list()
############### Number of Topics
gene_vals <- c(25000)#c(500,1000,5000,10000,25000)
K <- 12
############ ###
...
results <- matrix(0,nrow = length(gene_vals),ncol = n_col)
...
z <- 1
for(n_gene in gene_vals){#length(scenarios)){
  
  print(paste("NGenes: ",n_gene,sep = ''))
  ...
  #Store models
  models[[n_gene]] <- list(spe_train,mlp)
  ############Predict Test###################
  test_res <- c()
